...
export const CitaServices = {
    ...
    async validarDisponibilidad(fechaHoraInicio, fechaHoraFinNueva, idprofesional) {
        ...
        const existeCruce = citas.some((cita) => {
            if (!cita.fechaHora || !cita.FechaHoraFin)
                return false;
            return (fechaHoraInicio < cita.FechaHoraFin &&
    ...
    async calcularMontoServicio(idservicio) {
    ...
    async solicitar(data) {
        ...
        const fechaHoraFin = new Date(fechaHora.getTime() + duracionHoras * 60 * 60 * 1000);
        const horaFin = fechaHoraFin.toTimeString().slice(0, 5);
        await CitaServices.validarDisponibilidad(fechaHora, fechaHoraFin, idProfesional);
    ...
    async tieneCitaActiva(idprofesional) {
    ...
    async aceptar(id, motivo) {
        ...
        if (ocupado) {
            throw AppError.badRequest("No puedes aceptar esta cita hasta finalizar la anterior");
        }
    ...
    async cancelar(id, motivo, actorRol) {
    ...
    async completar(id) {
        ...
        const tiempoTotalMinutos = Math.round((ahora.getTime() - cita.fechaHora.getTime()) / 60000);
