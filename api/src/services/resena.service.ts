...
export const ResenaService = {
  ...
  async getAll(profesionalId: number) {
...
return prisma.resena.findUnique({
  ...
  async validarResenaUnica(citaId: number) {
  ...
  if (resenaExistente) {
    throw AppError.badRequest(
      "Esta cita ya cuenta con una reseña registrada, no se puede crear otra",
    );
  }
},
  ...
  if (cita.Estado !== ESTADOCITA.COMPLETA) {
    throw AppError.badRequest(
      "Solo se pueden dejar reseñas de citas completadas",
    );
  }
},
